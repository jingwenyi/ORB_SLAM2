...
//ÓÃg2o ÓÅ»¯ÁË¹Ø¼üÖ¡µÄÎ»×ËºÍµØÍ¼µãµÄ×ø±ê
void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, 	//¹Ø¼üÖ¡
									const vector<MapPoint *> &vpMP,    	//µØÍ¼µã
                                 	int nIterations, 					//µü´ú´ÎÊı
                                 	bool* pbStopFlag, 					//ÊÇ·ñÇ¿ÖÆÔİÍ£
                                 	const unsigned long nLoopKF, 		//loop  ¹Ø¼üÖ¡¸öÊı
                                 	const bool bRobust)					//ÊÇ·ñÊ¹ÓÃºËº¯Êı
{
       ...
       const map<KeyFrame*,size_t> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(pKF->mvuRight[mit->second]<0)
            {
				...
				//°Ñmappoint ¶ÔÓ¦µÄ ¹Ø¼üµã×ø±ê¸³Öµ¸øobs
...
//ÓÃg2o ½øĞĞÓÅ»¯£¬Ö»ÓÅ»¯Î»×Ë£¬²»ÓÅ»¯µØÍ¼µã
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF,		//¹Ø¼üÖ¡ 
											bool* pbStopFlag, 	//ÊÇ·ñÍ£Ö¹ÓÅ»¯±êÖ¾
											Map* pMap)			//ÔÚÓÅ»¯ºó£¬¸üĞÂ×´Ì¬Ê±ĞèÒªÓÃµ½map µÄ»¥³âÁ¿mMutexMapUpdate
{    
    ...
    for(int i=0, iend=vNeighKFs.size(); i<iend; i++)
    {
		...
		//ÉèÖÃba µÄ¹Ø¼üÖ¡id £¬µ±Ç°¹Ø¼üÖ¡
        pKFi->mnBALocalForKF = pKF->mnId;
		//°Ñ¸Ã¹Ø¼üÖ¡²åÈë  lLocalKeyFrames ¶ÓÁĞ
        if(!pKFi->isBad())
        ...
        for(vector<MapPoint*>::iterator vit=vpMPs.begin(), vend=vpMPs.end(); vit!=vend; vit++)
        {
            ...
            if(pMP)
                if(!pMP->isBad())
                    ...
                    {
                    	//°ÑËùÓĞµØÍ¼µã²åÈëµ½lLocalMapPoints ¶ÓÁĞ
                        lLocalMapPoints.push_back(pMP);
						//ÉèÖÃ±êÖ¾Î»£¬±ê¼ÇÎª¾Ö²¿¹Ø¼üÖ¡( ÏàÁÚ¹Ø¼üÖ¡)
    ...
    //¹Ì¶¨¹Ø¼üÖ¡£¬²»ÊôÓÚ¾Ö²¿¹Ø¼üÖ¡£¬¼´ÏàÁÚ¹Ø¼üÖ¡£¬ÔÚÓÅ»¯µÄ¹ı³ÌÖĞ²»ÓÅ»¯
    list<KeyFrame*> lFixedCameras;
	//±éÀúËùÓĞµÄµØÍ¼µã
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
    	//»ñÈ¡ÄÜ¹»¹Û²âµ½¸ÃµØÍ¼µãµÄËùÓĞ¹Ø¼üÖ¡
        map<KeyFrame*,size_t> observations = (*lit)->GetObservations();

		//±éÀúÄÜ¹»¹Û²âµ½¸ÃµØÍ¼µãµÄËùÓĞ¹Ø¼ü
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
        	//»ñÈ¡¸Ã¹Ø¼üÖ¡
            KeyFrame* pKFi = mit->first;

			//¸ÃÖ¡²»ÊÇÏàÁÚ¹Ø¼üÖ¡£¬Ò²²»ÊÇ¹Ì¶¨¹Ø¼üÖ¡
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
		...
		//»ñÈ¡ÄÜ¹»¹Û²âµ½¸ÃµØÍ¼µãµÄ¹Ø¼üÖ¡
        const map<KeyFrame*,size_t> observations = pMP->GetObservations();

        //Set edges
        //¶ÔµØÍ¼µãºÍÆä¹ØÁªµÄ¹Ø¼üÖ¡£¬½¨Á¢±ß
        //±éÀúÄÜ¹»¹Û²âµ½¸ÃµØÍ¼µãµÄ¹Ø¼üÖ¡
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {   
            	//»ñÈ¡¸Ã¹Ø¼üÖ¡Óë¸ÃµØÍ¼µã¶ÔÓÚµÄ¹Ø¼üµã
    ...
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        ...
        if(pMP->isBad())
            continue;
		//»ùÓÚ¿¨·½¼ì²â¼ÆËã³öãĞÖµ( ¼ÙÉè²âÁ¿ÓĞÒ»¸öÏñËØÆ«²î )
    ...
    // Check inlier observations       
    //ÔÚÓÅ»¯ºóÖØĞÂ¼ÆËãÎó²î£¬ÌŞ³ıÁ¬½ÓÎó²î±È½Ï´óµÄÖ¡ºÍµØÍ¼µã
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        g2o::EdgeSE3ProjectXYZ* e = vpEdgesMono[i];
        MapPoint* pMP = vpMapPointEdgeMono[i];
	...
	//Á¬½ÓÆ«²î±È½Ï´ó£¬ÔÚ¹Ø¼üÖ¡ÖĞÌŞ³ı¶Ô¸ÃµØÍ¼µãµÄ¹Û²â
	//Á¬½ÓÆ«²î±È½Ï´ó£¬ÔÚµØÍ¼µãÖĞÌŞ³ı¶Ô¸Ã¹Ø¼üÖ¡µÄ¹Û²â
    if(!vToErase.empty())
    {
    ...
    //Keyframes
    //ÓÅ»¯ºó¸üĞÂ¹Ø¼üÖ¡Î»×ËºÍµØÍ¼µãÎ»ÖÃ¡¢Æ½¾ù¹Û²â·½ÏòµÈÊôĞÔ
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        KeyFrame* pKF = *lit;
        g2o::VertexSE3Expmap* vSE3 = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKF->mnId));
